...
import { colors, fonts } from "./theme.json";
...
export const Spacer = styled.View`
  ...
  border: ${(props) => (props.border ? `1px solid ${colors.gray50}` : "none")};
`;

export const Divider = styled.View`
  width: 100%;
  height: ${(props) => props.size || "1px"};
  margin: ${(props) => props.spacing || "30px 0px 0px 0px"};
  border: ${(props) =>
    props.border ? `1px solid ${colors.gray50}50` : "none"};
`;

export const Input = styled.TextInput.attrs({
  placehlderTextColor: colors.muted,
})`
  display: flex;
  width: 100%;
  padding: 13px 15px;
...
export const Cover = styled.ImageBackground.attrs((props) => ({
  ...
  margin: ${(props) => props.spacing || 0};
  border-radius: ${(props) =>
    props.radius || props.circle ? props.radius || "60px" : "3px"};
  border: ${(props) => props.border || "none"};
